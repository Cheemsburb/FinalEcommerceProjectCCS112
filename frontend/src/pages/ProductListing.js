...
function ProductListing() {
  // 1. State for API data and UI status
  ...
  // 2. Fetch Data from Docker Backend (Port 8000)
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setProducts(data);    // Initialize display list
        setIsLoading(false);
      } catch (err) {
  ...
  // 3. Filtering Logic (Depends on allProducts, not json)
  useEffect(() => {
    ...
    filtered = filtered.filter(item => 
      Number(item.price) >= priceRange[0] && Number(item.price) <= priceRange[1]
    );
    ...
    setCurrentPage(1);
  }, [selectedBrands, priceRange, selectedSizes, selectedCategories, allProducts, isLoading]);
  ...
  if (isLoading) return <div className={styles.loadingContainer}>Loading products...</div>;
  if (error) return <div className={styles.errorContainer}>Error loading products: {error}</div>;
