...
import productsData from "../assets/products.json"; // for brand/model mapping
...
function Profile() {
    ...
    const addAddress = () => setAddresses([...addresses, { id: null, shippingAddress: "", state: "", zipCode: "" }]);
    ...
    const removeAddress = (index) => {
        ...
        } else setAddresses(addresses.filter((_, i) => i !== index));
    };
    ...
    const handleSaveChanges = async (e) => {
        ...
        for (const addr of addresses) {
            const payload = { address: addr.shippingAddress, state: addr.state, zip_code: addr.zipCode };
            const result = addr.id === null
    ...
    const handleLogout = (callApi = true) => {
        ...
        if (callApi && apiToken) makeAuthRequest("/logout", "POST");
            ...
            <div className={style.leftColumn}>
                <form onSubmit={handleSaveChanges}>
                    ...
                    <div className={style.formGrid}>
                        <div className={style.formGroup}><label>First Name</label><input type="text" value={firstName} readOnly /></div>
                        <div className={style.formGroup}><label>Last Name</label><input type="text" value={lastName} readOnly /></div>
                        <div className={style.formGroup}><label>Email</label><input type="email" value={email} readOnly /></div>
                        <div className={style.formGroup}><label>Phone</label><input type="tel" value={phone} onChange={(e)=>setPhone(e.target.value)} /></div>
                    </div>

                    <h2 className={style.sectionTitle}>Change Password</h2>
                    <div className={style.formGrid}>
                        <div className={style.formGroup}><label>New Password</label><input type="password" value={newPassword} onChange={e=>setNewPassword(e.target.value)} autoComplete="new-password" /></div>
                        <div className={style.formGroup}><label>Confirm New Password</label><input type="password" value={confirmPassword} onChange={e=>setConfirmPassword(e.target.value)} autoComplete="new-password" /></div>
                    </div>

                    <h2 className={style.sectionTitle}>Addresses</h2>
                    <table className={style.addressTable}>
                        <thead><tr><th>Shipping Address</th><th>State</th><th>Zip Code</th><th>Action</th></tr></thead>
                        <tbody>
                            {addresses.length===0 && <tr><td colSpan="4" className={style.emptyMessage}>No addresses saved. Click "Add Another Address" to start.</td></tr>}
                    ...
                    <table className={style.orderTable}>
                        ...
                        <tbody>
                            {orders.length>0 ? orders.map((order, idx)=>{
                                return order.items && order.items.length>0 ? order.items.map((item,i)=>{
                                    const product = productsData.find(p=>p.id===item.product_id);
                                    const brand = product?.brand || "Unknown";
                                    const model = product?.model || "Unknown";
                                    const total = Number(item.price_at_purchase)*Number(item.quantity);
                                    return (
                                        <tr key={`${idx}-${i}`}>
                                            <td>{brand}</td>
                                            <td>{model}</td>
                                    ...
                                    <tr key={idx}><td colSpan="4" className={style.emptyMessage}>No items in this order</td></tr>
                                );
